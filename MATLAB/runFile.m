...
% Flags
flags.Opt = 1;          % 0 - single run, 1 - optimization
flags.ConFail = 0;      % 1 - use structural failure as a constraint on optimization
flags.ConWireCont = 0;  % 1 - use wire length continuity as a constraint to set appropriate wire forces in multi-point optimizations
flags.ConJigCont = 0;   % 1 - use jig continuity
flags.ConDef = 0;       % 1 - constraints on maximum deformation of the rotor
flags.MultiPoint = 4;   % 0 - single point optimization, 1 - 4 point optimization (h=0.5, h=3, wind case, gravity load)
flags.Quad = 1;         % 0 - prop drive, 1 - quad rotor
flags.FreeWake = 1;     % 0 - momentum theory, 1 - free vortex ring wake
flags.PlotWake = 0;     % 0 - don't plot wake, 1 - plot wake 
flags.DynamicClimb = 0; % 0 - vc imposes downward velocity, 1 - vc represents climb (final altitude depends on Nw)
flags.Cover = 0;        % 0 - no cover over root rotor blades, 1 - cover
flags.Load = 0;         % 0 - normal run, 1 - gravity forces only, 2 - prescribed load from pLoad
flags.Cdfit = 1;        % 0 - analytic model for drag coefficient, 1 - curve fit on BE airfoils
flags.GWing = 1;        % 0 - Daedalus style wing, 1 - Gossamer style wing (changes amount of laminar flow)
...
flags.CFRPType = 1;     % 1 - high modulus carbon fibre
flags.WireType = 1;     % 1 - Steel, 2 - Vectran (material for lift wire)
...
% Possible lift distributions on Atlas
Cl_  = [1.4 1.3 1.1]; % 0.5 m No canard lift distribution (as designed)
Omega = 0.165*2*pi; %rad/s

Cl_  = [1.5 1.43 1.23]; % 0.5 m No canard lift distribution (as flown)
Omega = 0.165*2*pi; %rad/s

% Cl_  = [1.4 1.25 0.8]; % 0.5 m Canard at negative aoa (causes twist of 2.25deg)
% Omega = 0.195*2*pi; %rad/s

%Cl_  = [1.45 1.4 1.45]; % 0.5 m Canard at positive aoa (causes twist of 2.25deg)
%Omega = 0.165*2*pi; %rad/s
% Cl_  = [1.2 1 1.15]; % 3.5 m canard at positive aoa (causes twist of 2.25deg)
% Omega = 0.186*2*pi; %rad/s
% Cl_  = [1.25 1.12 1.24]; % 2.5 m canard at positive aoa (causes twist of 2.25deg)
% Omega = 0.186*2*pi; %rad/s
% Cl_  = [1.33 1.24 1.33]; % 1.5 m canard at positive aoa (causes twist of 2.25deg)
% Omega = 0.186*2*pi; %rad/s

% Climb 0.3 m/s
% Cl_  = [0.8 1.05 1.3]; % 0.5 m Canard at positive aoa (causes twist of 2.25deg)
% Omega = 0.165*2*pi; %rad/s
% Cl_  = [1.15 0.85 1.11]; % 3.5 m canard at positive aoa (causes twist of 2.25deg)
% Omega = 0.186*2*pi; %rad/s
% Cl_  = [0.85 0.75 1.14]; % 2.5 m canard at positive aoa (causes twist of 2.25deg)
% Omega = 0.186*2*pi; %rad/s
% Cl_  = [0.83 0.9 1.23]; % 1.5 m canard at positive aoa (causes twist of 2.25deg)
% Omega = 0.186*2*pi; %rad/s
...
mPilot = 71; %kg
presLoad = []; %prescribed load distribution


% Helicopter properties for a Quad-Rotor design
if flags.Quad
    ...
    vrCon.FOSwire = 0.5; %2;
        ...
        d_   =   [3.442    1.99    1.239   ]*2.54/100; %in -> m
...
%         anhedral = 0;
%         
        %Spar test 2 (validation of failure test)
%         vrCon.Wind = 10/3.6;
...
%         anhedral = 0;
        ...
        % Decide which variables to optimize and their min/max values
        if flags.Opt

            i = 1;

            vrOpt(i).name = 'Omega';
            vrOpt(i).u = 0.25*2*pi; %25
...
%             i = i+1;
    ...
    %         i = i+1;
            ...
            % Linked parameters
            % any parameters linked to an optimization variable
            i = 1;
%             vrLink(i).equation = 'Cl_(3) = Cl_(2);';
%             i = (i+1);
               vrLink(i).equation = 'blah = 0;'; %required to have at least one parameter
               i = (i+1);
            ...
            % When doing a multipoint optimization we need a different
            % rotor speed and lift distribution at altitude
            ...
            % jig constraint didn't work well when trying to match Cl over
            % three points (instead just set constraint that Cl_(3) =
            % Cl_(2)
%             vrOptH(i).name = 'Cl_';
...
%             i = i+1;
     ...
     %%%%%%%%%%%%%%%%%%%%%%%%
     % EVERYTHING BELOW WAS USED FOR PAST OPTIMIZATIONS, SETTING UP
     % DIFFERENT SETS OF VARIABLES FOR DIFFERENT SIZED ROTORS AND FOR A
